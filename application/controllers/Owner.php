...
class Owner extends CI_Controller
{
    ...
    public function laporanbahan()
    {
        ...
        //$data['sum'] = $this->admin_model->print_bahan($tgl_pembelian); 
        $data ['title'] = 'Laporan Pembelian Bahan';
        $this->template->load('templates/master', 'backend/owner/laporanbahan', $data);
    }

    public function laporantransaksi()
    {
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $data['tahun'] = $this->admin_model->gettahuntransaksi();
        //$data['sum'] = $this->admin_model->print_bahan($tgl_pembelian); 
        $data ['title'] = 'Laporan Transaksi';
        $this->template->load('templates/master', 'backend/owner/laporantransaksi', $data);
    }
    ...
    public function filter(){
        ...
        if ($nilaifilter == 1) {
            $data['title'] = "Laporan Pengeluaran Berdasarkan Tanggal";
            $data['subtitle'] = "Dari Tanggal : ".$tanggalawal.' Sampai Tanggal : '.$tanggalakhir;
            $data['datafilter'] = $this->admin_model->filterbytanggal($tanggalawal,$tanggalakhir);
            //$data['sum'] = $this->admin_model->print_bahan($tgl_pembelian);
            //$this->template->load('backend/owner/print_bahan', $data);
            $this->load->view( 'backend/owner/print_bahan', $data);

        } elseif ($nilaifilter == 2) {
            $data['title'] = "Laporan Pengeluaran Berdasarkan Bulan";
            $data['subtitle'] = "Dari Bulan : ".$bulanawal.'Sampai Bulan :'.$bulanakhir.' Tahun : '.$tahun1;
            $data['datafilter'] = $this->admin_model->filterbybulan($tahun1, $bulanawal,$bulanakhir);
            //$data['sum'] = $this->admin_model->print_bahan($tgl_pembelian);
            //$this->template->load('backend/owner/print_bahan', $data);
            $this->load->view( 'backend/owner/print_bahan', $data);

        } elseif ($nilaifilter == 3) {
            $data['title'] = "Laporan Pengeluaran Berdasarkan Tahun";
            $data['subtitle'] = ' Tahun : '.$tahun2;
            $data['datafilter'] = $this->admin_model->filterbytahun($tahun2);
            //$data['sum'] = $this->admin_model->print_bahan($tgl_pembelian);
            //$this->template->load('backend/owner/print_bahan', $data);
            $this->load->view( 'backend/owner/print_bahan', $data);
    ...
    public function filtertransaksi(){
        ...
        if ($nilaifilter == 1) {
            $data['title'] = "Laporan Transaksi Berdasarkan Tanggal";
            $data['subtitle'] = "Dari Tanggal : ".$tanggalawal.' Sampai Tanggal : '.$tanggalakhir;
            $data['transaksi'] = $this->admin_model->filtertransaksibytanggal($tanggalawal,$tanggalakhir);
            $this->load->view( 'backend/owner/print_transaksi', $data);

        } elseif ($nilaifilter == 2) {
            $data['title'] = "Laporan Transaksi Berdasarkan Bulan";
            $data['subtitle'] = "Dari Bulan : ".$bulanawal.'Sampai Bulan :'.$bulanakhir.' Tahun : '.$tahun1;
            $data['transaksi'] = $this->admin_model->filtertransaksibybulan($tahun1, $bulanawal,$bulanakhir);
            $this->load->view( 'backend/owner/print_transaksi', $data);

        } elseif ($nilaifilter == 3) {
            $data['title'] = "Laporan Transaksi Berdasarkan Tahun";
            $data['subtitle'] = ' Tahun : '.$tahun2;
            $data['transaksi'] = $this->admin_model->filtertransaksibytahun($tahun2);
            $this->load->view( 'backend/owner/print_transaksi', $data);
